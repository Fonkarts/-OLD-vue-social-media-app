          ...
          if(req.body.data.username) {
            ...
            .then(() => {
              ...
              .then(() => {
                ...
                .then(() => {
                  // Met à jour le nom de l'utilisateur pour les articles concernés
                  Comment.update(req.body.data, {where: {userId: req.params.id}})
                  .then(() => res.status(200).json({message: "Nom de l'utilisateur mis à jour (tables users, articles et comments) !"}))
                  .catch(() => res.status(500).json({message: "Problème avec la mise à jour de la table comments !"}))
                })
                .catch(() => res.status(404).json({message: "Commentaire non trouvé !"}))
              })
              .catch(() => res.status(500).json({message: "Problème avec la mise à jour de la table articles !"}));
            })
